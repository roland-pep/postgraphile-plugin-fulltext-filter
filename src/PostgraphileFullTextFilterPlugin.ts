...
import { Build, Inflection } from "graphile-build";
import {
  GraphQLObjectType,
  GraphQLEnumType,
  GraphQLScalarType,
...
import { Context } from "postgraphile-core";

const tsquery = new Tsquery();
export default function PostGraphileFulltextFilterPlugin(builder: Build): void {
  ...
  builder.hook("build", (build: Build) => {
    ...
    const scalarName = build.inflection.fullTextScalarTypeName();
  ...
  builder.hook("init", (_: any, build: Build) => {
    const {
  ...
  builder.hook(
    ...
    (fields: GraphQLObjectType, build: Build, context) => {
      const {
        pgIntrospectionResultsByKind: introspectionResultsByKind,
